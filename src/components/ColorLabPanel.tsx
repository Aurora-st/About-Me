...
const ColorLabPanel = React.forwardRef<HTMLDivElement, ColorLabPanelProps>(({ isOpen, onClose }, ref) => {
  ...
  const itemVariants = {
    ...
    show: { opacity: 1, y: 0, filter: "blur(0px)", transition: { type: "spring" as const, stiffness: 130, damping: 16 } },
  };

  const panelInteriorVariants = {
    hidden: { opacity: 0 },
    show: {
      opacity: 1,
      transition: {
  ...
  const headerVariants = {
    ...
    show: { opacity: 1, y: 0, filter: "blur(0px)", transition: { type: "spring" as const, stiffness: 120, damping: 15 } },
  };

  const subHeaderVariants = {
    hidden: { opacity: 0, y: -5, filter: "blur(1px)" },
    show: { opacity: 1, y: 0, filter: "blur(0px)", transition: { type: "spring" as const, stiffness: 120, damping: 15 } },
  };

  const tabVariants = {
    hidden: { opacity: 0, scaleY: 0, filter: "blur(2px)" },
    show: { opacity: 1, scaleY: 1, filter: "blur(0px)", transition: { type: "spring" as const, stiffness: 140, damping: 16 } },
  };
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          dragElastic={0.05}
          dragMomentum={false}
          initial={{ opacity: 0, scale: 0.01, y: 180, x: 120, rotateX: 20, filter: "blur(12px) brightness(2.5)" }}
          animate={{ opacity: 1, scale: 1, y: 0, x: 0, rotateX: 0, filter: "blur(0px) brightness(1)" }}
          exit={{ opacity: 0, scale: 0.01, y: 180, x: 120, rotateX: 20, filter: "blur(12px) brightness(2.5)" }}
          transition={{ type: "spring", stiffness: 110, damping: 19, delay: 0.15 }}
        ...
        >
          ...
          {/* Staggered interior container */}
          <motion.div
            variants={panelInteriorVariants}
            initial="hidden"
            animate="show"
          ...
          >
            ...
            <motion.div 
              ...
              className="px-3 py-1 bg-zinc-950/50 border-b border-zinc-850 flex items-center justify-between text-[8px] font-mono text-zinc-500"
            >
            ...
            {!isMinimized && (
              <>
                ...
                <motion.div 
                  ...
                  className="flex border-b border-zinc-800 text-[10px] font-mono"
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 custom-scrollbar">
                  ...
                  {activeTab === "spectral" && (
                    ...
                    >
                      ...
                      <motion.div variants={itemVariants} className="mt-2 bg-zinc-950/80 p-2.5 border border-zinc-850 rounded flex flex-col gap-1.5">
                  ...
                  {activeTab === "shaders" && (
                    ...
                    >
                      ...
                      <motion.div variants={itemVariants} className="bg-zinc-950/80 p-2.5 rounded border border-zinc-900/60 flex gap-2 items-start mt-2">
                  ...
                  {activeTab === "presets" && (
                    <motion.div 
                      ...
                      animate="show"
                    >
                      <motion.span variants={itemVariants} className="text-[8px] font-mono text-zinc-500 tracking-widest uppercase mb-1">
                ...
                <motion.div 
                  ...
                  className="p-3 bg-zinc-950/80 border-t border-zinc-900 flex justify-between gap-3"
                >
                  <button
